@testset "Positional shortcuts" begin
    ...
    @test @vlplot(:point, {:a}, data = df) == @vlplot(:point, x = :a, data = df)
    @test @vlplot(:point, {:a}, {:b}, data = df) ==
          @vlplot(:point, x = :a, y = :b, data = df)
    @test @vlplot(:point, {"a:q"}, {"b:q"}, data = df) ==
          @vlplot(:point, x = "a:q", y = "b:q", data = df)
