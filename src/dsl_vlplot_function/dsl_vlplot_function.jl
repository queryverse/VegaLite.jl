...
function vlfrag(args...; kwargs...)
    return VLFrag(
        ...
        OrderedDict{String,Any}(
            string(k) => Vega.convert_nt_to_dict(v, VLFrag) for (k, v) in kwargs
        ),
    )
end
...
fix_shortcut_level_encoding(name, spec_frag::Symbol, unnamed_inline_data) =
    VLFrag(Any[], OrderedDict{String,Any}("field" => string(spec_frag)))

fix_shortcut_level_encoding(name, spec_frag::String, unnamed_inline_data) =
    VLFrag(Any[], OrderedDict{String,Any}(parse_shortcut(spec_frag)...))
...
function fix_shortcut_level_encoding(name, spec_frag::AbstractVector, unnamed_inline_data)
    if !(name in ("tooltip", "detail", "order"))
        ...
        return VLFrag(
            Any[],
            OrderedDict{String,Any}("field" => string(name), "title" => nothing),
        )
    else
        return [
            fix_shortcut_level_encoding(name, i, unnamed_inline_data) for i in spec_frag
        ]
    end
end
...
function fix_shortcut_level_encoding(name, spec_frag::VLFrag, unnamed_inline_data)
    ...
    if length(spec_frag.positional) == 1
        ...
        elseif spec_frag.positional[1] isa AbstractVector &&
               !(string(name) in ("tooltip", "detail", "order"))
            if haskey(spec, "field")
                error(
                    "The $name encoding channel cannot have inline data and a `field` element.",
                )
            end
    ...
    if !isempty(spec_frag.positional)
        ...
        if haskey(spec_frag.named, "url")
            ...
            elseif spec["url"] isa URI
                ...
                spec["url"] =
                    Sys.iswindows() && spec["url"].scheme == "file" ?
                    as_uri[1:5] * as_uri[7:end] : as_uri
            end
...
function fix_shortcut_level_data(spec_frag::AbstractPath)
    ...
    return VLFrag(
        [],
        OrderedDict{String,Any}(
            "url" => Sys.iswindows() ? as_uri[1:5] * as_uri[7:end] : as_uri,
        ),
    )
end
...
function fix_shortcut_level_data(spec_frag::URI)
    ...
    return VLFrag(
        ...
        OrderedDict{String,Any}(
            "url" =>
                Sys.iswindows() && spec_frag.scheme == "file" ?
                as_uri[1:5] * as_uri[7:end] : as_uri,
        ),
    )
end
...
function fix_shortcut_level_spec(spec_frag::VLFrag)
    ...
    if length(spec_frag.positional) in (2, 3)
        ...
        if haskey(spec["encoding"].named, "x")
            error(
                "One cannot specify both a named and a positional argument for the `x` encoding channel.",
            )
        end
    ...
    if length(spec_frag.positional) == 3
        if haskey(spec["encoding"].named, "y")
            error(
                "One cannot specify both a named and a positional argument for the `y` encoding channel.",
            )
        end
    ...
    if haskey(spec, "transform")
        for transform in spec["transform"]
            if haskey(transform.named, "from") &&
               haskey(transform.named["from"].named, "data")
                transform.named["from"].named["data"] =
                    fix_shortcut_level_data(transform.named["from"].named["data"])
            end
        end
    end
    ...
    elseif !isempty(inline_unnamed_data)
        if !all(
            isequal(length(inline_unnamed_data[1][2])),
            map(i -> length(i[2]), inline_unnamed_data),
        )
        ...
        spec["data"] = VLFrag(
            [],
            OrderedDict{String,Any}(
                "values" => Vega.DataValuesNode(
                    TableTraitsUtils.create_tableiterator(
                        map(i -> i[2], inline_unnamed_data),
                        map(i -> i[1], inline_unnamed_data),
                    ),
                ),
            ),
        )
...
function convert_frag_tree_to_dict(spec::VLFrag)
    ...
    spec_as_dict = OrderedDict{String,Any}(
        k => Vega.replace_remaining_frag(v) for (k, v) in spec.named
    )

    spec_as_dict2 = Vega.walk_dict(spec_as_dict, "root") do p, parent
        if p[1] == "typ"
            Base.depwarn(
                "`typ` in VegaLite.jl specs is deprecated, use `type` instead.",
                :vlplot,
            )
...
function vlplot(args...; kwargs...)
    return VLSpec(
        convert_frag_tree_to_dict(fix_shortcut_level_spec(vlfrag(args...; kwargs...))),
    )
end
