...
function pushpars!(pars::Dict{String,Any}, val, prop::Symbol=Symbol())
  if prop == Symbol()
    ...
    isa(val, VLSpec) && sprop!=vltype(v) && error("expecting function $(jlfunc(sprop)) for keyword arg $prop, got $(vltype(val))")
  end
